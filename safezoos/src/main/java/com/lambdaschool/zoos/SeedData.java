...
public class SeedData implements CommandLineRunner
{
    ...
    public void run(String[] args) throws Exception
    {
        ...
        User u2 = new User("andrew", "badpassword", zoodata);
        userrepos.save(u2);

        ArrayList<UserRoles> animaldata = new ArrayList<>();
        animaldata.add(new UserRoles(new User(), r3));
        User u3 = new User("jessica", "badpassword", animaldata);
        userrepos.save(u3);

        ArrayList<UserRoles> mgrdata = new ArrayList<>();
        mgrdata.add(new UserRoles(new User(), r4));
        User u4 = new User("russell", "badpassword", mgrdata);
        userrepos.save(u4);

        admins = new ArrayList<>();
        admins.add(new UserRoles(new User(), r1));
        User u5 = new User("ruby", "badpassword", admins);
        userrepos.save(u5);
    }
}